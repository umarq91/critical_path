...
import { listBrandsForExport } from "@/data/brands";
...
import { BRAND_GRID_COLUMN_ORDER, BRAND_RECORD_COLUMN_GROUPS } from "@/app/(app)/brands/export/brand-record-columns";
...
// The Brands admin page's export — always exactly one dataset (Brand Records), same shape as
// tasks/export/route.ts: every query param either shapes the table's rows (filters/sort, the
// board's own current state) or its columns.
export async function GET(request: NextRequest) {
  ...
  const columnKeys = requestedColumns.length > 0 ? requestedColumns : defaultColumnKeys(BRAND_RECORD_COLUMN_GROUPS);
  const columns = selectColumns(BRAND_RECORD_COLUMN_GROUPS, columnKeys, BRAND_GRID_COLUMN_ORDER);
  if (columns.length === 0) return errorResponse(400, "columns must include at least one valid column");

  const brandRecords = await listBrandsForExport({ filters, sortBy, sortDir });
  const sheet: ExportSheet = { name: "Brands", columns, rows: brandRecords.data } as ExportSheet;

  const hasFilters = Object.keys(filters).length > 0;
  const filename = buildExportFilename("brands", format, hasFilters ? "filtered" : undefined);
  const headers = new Headers({
    "Content-Disposition": `attachment; filename="${filename}"`,
    "X-Export-Truncated": String(brandRecords.truncated),
  });
