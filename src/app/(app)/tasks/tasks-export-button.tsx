...
function filenameFromDisposition(disposition: string | null, format: ExportFormat) {
  ...
  return match?.[1] ?? `tasks-export.${format}`;
}

interface TasksExportButtonProps {
  /** The current page's row count for the grid's active filters — export always targets that
   *  same scope, so an empty grid means nothing to export. */
  rowCount: number;
}

// Exports whatever the grid is currently showing — same filters, search term and sort the
// board itself is reading off the URL (useDataTableQueryState, same hook tasks-board.tsx uses),
// forwarded to tasks/export/route.ts rather than re-derived. This replaced a decorative
// "Export as CSV / Export as PDF" dropdown that had no handlers.
export const TasksExportButton = ({ rowCount }: TasksExportButtonProps) => {
  const { params } = useDataTableQueryState(TASKS_QUERY_STATE);
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      const response = await fetch(`/tasks/export?${searchParams.toString()}`);
      if (!response.ok) {
      ...
      toast.success(truncated ? "Export downloaded — row limit reached, file is a partial export" : "Tasks exported");
      setOpen(false);
    } catch (error) {
  ...
  return (
    <FormDialog
      title="Export Tasks"
      description="Exports the tasks matching your current filters and search."
      trigger={
