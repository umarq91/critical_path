...
import { SEASON_RECORD_COLUMN_GROUPS } from "@/app/(app)/seasons/export/season-record-columns";
...
const DEFAULT_COLUMN_KEYS = defaultColumnKeys(SEASON_RECORD_COLUMN_GROUPS);
const ALL_COLUMN_KEYS = allColumnKeys(SEASON_RECORD_COLUMN_GROUPS);

function filenameFromDisposition(disposition: string | null, format: ExportFormat) {
  const match = disposition ? /filename="?([^"]+)"?/.exec(disposition) : null;
  return match?.[1] ?? `seasons-export.${format}`;
}

interface SeasonsExportButtonProps {
...
// URL (useDataTableQueryState, same hook seasons-board.tsx uses), forwarded to
// seasons/export/route.ts rather than re-derived. Mirrors tasks/tasks-export-button.tsx; this
// replaced a decorative "Export as CSV / Export as PDF" dropdown that had no handlers.
export const SeasonsExportButton = ({ rowCount }: SeasonsExportButtonProps) => {
  const { params } = useDataTableQueryState(SEASONS_QUERY_STATE);
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      const response = await fetch(`/seasons/export?${searchParams.toString()}`);
      if (!response.ok) {
      ...
      toast.success(truncated ? "Export downloaded — row limit reached, file is a partial export" : "Seasons exported");
      setOpen(false);
    } catch (error) {
  ...
  return (
    <FormDialog
      title="Export Seasons"
      description="Exports the seasons matching your current filters."
      trigger={
    ...
    >
      <div className="flex flex-col gap-5">
        ...
        <div className="flex flex-col gap-4">
          ...
          {SEASON_RECORD_COLUMN_GROUPS.map((group) => (
