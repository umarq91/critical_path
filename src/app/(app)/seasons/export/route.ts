...
import { listSeasonsForExport } from "@/data/seasons";
...
import { SEASON_GRID_COLUMN_ORDER, SEASON_RECORD_COLUMN_GROUPS } from "@/app/(app)/seasons/export/season-record-columns";
...
// The Seasons admin page's export — always exactly one dataset (Season Records), same shape
// as tasks/export/route.ts: every query param either shapes the table's rows (filters/sort,
// the board's own current state) or its columns.
export async function GET(request: NextRequest) {
  ...
  const columnKeys = requestedColumns.length > 0 ? requestedColumns : defaultColumnKeys(SEASON_RECORD_COLUMN_GROUPS);
  const columns = selectColumns(SEASON_RECORD_COLUMN_GROUPS, columnKeys, SEASON_GRID_COLUMN_ORDER);
  if (columns.length === 0) return errorResponse(400, "columns must include at least one valid column");

  const seasonRecords = await listSeasonsForExport({ filters, sortBy, sortDir });
  const sheet: ExportSheet = { name: "Seasons", columns, rows: seasonRecords.data } as ExportSheet;

  const hasFilters = Object.keys(filters).length > 0;
  const filename = buildExportFilename("seasons", format, hasFilters ? "filtered" : undefined);
  const headers = new Headers({
    "Content-Disposition": `attachment; filename="${filename}"`,
    "X-Export-Truncated": String(seasonRecords.truncated),
  });
