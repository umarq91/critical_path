...
function filenameFromDisposition(disposition: string | null, format: ExportFormat) {
  ...
  return match?.[1] ?? `key-stages-export.${format}`;
}

interface KeyStagesExportButtonProps {
...
// URL (useDataTableQueryState, same hook key-stages-board.tsx uses), forwarded to
// key-stages/export/route.ts rather than re-derived. Mirrors tasks/tasks-export-button.tsx;
// this replaced a decorative "Export as CSV / Export as PDF" dropdown that had no handlers.
export const KeyStagesExportButton = ({ rowCount }: KeyStagesExportButtonProps) => {
  const { params } = useDataTableQueryState(KEY_STAGES_QUERY_STATE);
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      const response = await fetch(`/key-stages/export?${searchParams.toString()}`);
      if (!response.ok) {
      ...
      toast.success(truncated ? "Export downloaded — row limit reached, file is a partial export" : "Key stages exported");
      setOpen(false);
    } catch (error) {
  ...
  return (
    <FormDialog
      title="Export Key Stages"
      description="Exports the key stages matching your current filters."
      trigger={
